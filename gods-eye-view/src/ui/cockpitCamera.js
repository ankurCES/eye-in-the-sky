...
export function update() {
  ...
  // Mission-sim UAV: roll the chase-cam with the drone's bank so turns read
  // like a flight sim. Rotate the up vector about the forward axis by the
  // reported roll angle (pitch is already blended into the fixed view pitch).
  const rollDeg = Number.isFinite(info?.rollDeg) ? info.rollDeg : 0;
  if (rollDeg !== 0) {
    const rollRad = Cesium.Math.toRadians(rollDeg);
    const cosR = Math.cos(rollRad);
    const sinR = Math.sin(rollRad);
    // Rodrigues' rotation of scratchUp about unit scratchForward.
    const fx = this.scratchForward.x;
    const fy = this.scratchForward.y;
    const fz = this.scratchForward.z;
    const ux = this.scratchUp.x;
    const uy = this.scratchUp.y;
    const uz = this.scratchUp.z;
    const dot = fx * ux + fy * uy + fz * uz;
    const cx = fy * uz - fz * uy;
    const cy = fz * ux - fx * uz;
    const cz = fx * uy - fy * ux;
    this.scratchUp.x = ux * cosR + cx * sinR + fx * dot * (1 - cosR);
    this.scratchUp.y = uy * cosR + cy * sinR + fy * dot * (1 - cosR);
    this.scratchUp.z = uz * cosR + cz * sinR + fz * dot * (1 - cosR);
    Cesium.Cartesian3.normalize(this.scratchUp, this.scratchUp);
  }
