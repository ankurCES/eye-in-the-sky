...
export function readAircraftInfo() {
  ...
  // A tracked UAV (mission-sim drone) short-circuits the flight/military
  // resolver: it isn't in either of those layers, and its descriptor already
  // matches the cockpit's expected info shape.
  const trackedId = trackedEntity?.gevTrackedId || '';
  if (String(trackedId).startsWith('uav:')) {
    const uavInfo = this.services.uavLayer?.getTrackedInfo?.();
    if (uavInfo) return uavInfo;
  }
