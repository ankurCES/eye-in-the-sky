...
export class CockpitCoordinator {
  constructor({
    ...
    this.cockpitView = new CockpitViewController(viewer, {
      ...
      isEntryAllowed: () => {
        // Mission-sim UAV: entering the cockpit on a tracked drone is allowed
        // regardless of the flights context-mode gate.
        const trackedId = String(
          this.viewer?.trackedEntity?.gevTrackedId || '',
        );
        if (trackedId.startsWith('uav:')) return true;
        return cockpitEntryAllowed({
